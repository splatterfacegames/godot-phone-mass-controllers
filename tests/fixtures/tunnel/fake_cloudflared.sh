...
# unregister | unregister_recover | named_ok | named_local | needs_config. Replays logs/ to stderr.
# error_429_once writes/reads a marker at $PMC_FAKE_STATE_FILE to fail only on the first run.
LOGS="$(dirname "$0")/logs"
if [ "$1" = "--version" ]; then
